...
function snapToSlot(date, step, timezone) {
  ...
  return _jsJoda.ZonedDateTime.ofInstant(_jsJoda.Instant.ofEpochMilli(Math.floor(_dates2.default.nativeTime(date) / roundTo) * roundTo), ZoneId.of(timezone));
}
...
function minToDate(min, date, now) {
  ...
  now = _dates2.default.hours(now, 0);
  now = _dates2.default.minutes(now, totalMins + min);
  now = _dates2.default.seconds(now, 0);
