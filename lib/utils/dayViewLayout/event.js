...
var Event = exports.Event = function () {
  function Event(data, props) {
    ...
    this.start = _dates2.default.nativeTime(startDate);
    this.end = _dates2.default.nativeTime(endDate);
...
function convertToTimezone(date, tzString) {
  var zoneId = _jsJoda.ZoneId.of(tzString);

  if (date instanceof _jsJoda.LocalDate) {
    return _jsJoda.ZonedDateTime.of(date, _jsJoda.LocalTime.MIDNIGHT, zoneId);
  } else if (date instanceof _jsJoda.LocalDateTime) {
    return _jsJoda.ZonedDateTime.of(date, zoneId);
  } else if (date instanceof _jsJoda.ZonedDateTime) {
  ...
  return _jsJoda.ZonedDateTime.now(zoneId);
}
...
function normalizeDates(startDate, endDate, _ref) {
  var showMultiDayTimes = _ref.showMultiDayTimes,
      timezone = _ref.timezone,
      getNow = _ref.getNow;

  var zonedStartDate = convertToTimezone(startDate, timezone);
  var zonedEndDate = convertToTimezone(endDate, timezone);
  ...
  var s = _dates2.default.hours(zonedStartDate, 12);
  var e = _dates2.default.hours(zonedEndDate, 12);
  var c = _dates2.default.hours(current, 12);

  // Current day is at the start, but it spans multiple days,
  // so we correct the end.
  if (_dates2.default.eq(c, s) && _dates2.default.lt(c, e)) {
    return [zonedStartDate, _dates2.default.endOf(zonedStartDate, 'day')];
  }

  // Current day is in between start and end dates,
  // so we make it span all day.
  if (_dates2.default.gt(c, s) && _dates2.default.lt(c, e)) {
  ...
  if (_dates2.default.gt(c, s) && _dates2.default.eq(c, e)) {
